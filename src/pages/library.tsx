      ...
      <main className="flex min-h-screen flex-col bg-gradient-to-tr from-[#313131] to-[#000000]">
        ...
        <div className="container items-center gap-10 px-4 py-10 justify-center max-w-6xl mx-auto grid grid-cols-1">
            ...
            <div className="text-2xl text-white">
              <div className="grid grid-cols-1 md:grid-cols-2 md:max-w-4xl px-2 gap-0 m-auto">
                {params.lib.map((meal: any) => 
                  <>
                    <MealSearchResult title={meal.MealName} id={meal.MealID} image={JSON.parse(meal.Response).image} restaurantChain={JSON.parse(meal.Response).restaurantChain} isdemo={isdemo} type={meal.MealType} />
                  </>
                )}
              </div>
            </div>
